...
void CDrawRGB24::Initialize(int SrcW,int SrcH)
{
	//if(m_hDIB == NULL)
	//{
	//	m_hDIB=DrawDibOpen();
...
//void CDrawRGB24::Initialize1280_720()
//{
//	Initialize(1280,720);
//}

#if 0
void CDrawRGB24::Draw(HWND hWnd,unsigned char * buffer,int SrcW,int SrcH)
{
	HDC hdc=::GetDC(hWnd);
	RECT rect;
	...
	if(m_hDIB!=NULL)
	{
		...
		DrawDibDraw(m_hDIB,
			...
			//SrcW,				 // src : width
			//SrcH,				 // src : height
			&m_lpBmpInfo->bmiHeader,			 // bmp header info
			buffer,					 // bmp data
			0,					 // src :left
			0,					 // src :top
			SrcW,				 // src : width
...
void CDrawRGB24::DrawOriginal(HWND hWnd,unsigned char * buffer,int SrcW,int SrcH)
{
	...
	int DstWidth  = DestRect.Width()/2;
	int DstHeight = DestRect.Height()/2;
	SetStretchBltMode(m_pMemDC->GetDC().GetSafeHdc(),STRETCH_HALFTONE);
    ...
    ::StretchDIBits(
		m_pMemDC->GetDC().GetSafeHdc(),
	//	hDCDst,
		0+DstWidth, 0, DstWidth, DstHeight,
		0, 0, SrcW, SrcH,
		buffer, m_lpBmpInfo, DIB_RGB_COLORS, SRCCOPY );
		...
		m_pMemDC->GetDC().GetSafeHdc(),
	//	hDCDst,
		0, 0+DstHeight, DstWidth, DstHeight,
		0, 0, SrcW, SrcH,
		buffer, m_lpBmpInfo, DIB_RGB_COLORS, SRCCOPY );
		...
		m_pMemDC->GetDC().GetSafeHdc(),
	//	hDCDst,
		0+DstWidth, 0+DstHeight, DstWidth, DstHeight,
		0, 0, SrcW, SrcH,
		buffer, m_lpBmpInfo, DIB_RGB_COLORS, SRCCOPY );
	...
	//	0, 0, SrcW*2, SrcH,
	//	buffer, m_lpBmpInfo, DIB_RGB_COLORS, SRCCOPY );
   TextOut(m_pMemDC->GetDC().GetSafeHdc(),0,0,"钱波在这里做了一个测试",22);

	BitBlt(hDCDst,0,0,DestRect.Width(),DestRect.Height(),m_pMemDC->GetDC().GetSafeHdc(),0,0,SRCCOPY);
